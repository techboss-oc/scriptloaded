  ...
  <section class="admin-content flex-1 space-y-8 overflow-y-auto px-4 py-6 sm:px-6 lg:px-10" data-admin-content>
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-white/70">Approve, edit or retire catalog assets.</p>
      </div>
    ...
    <form method="get" class="admin-panel flex flex-wrap items-center gap-3 rounded-2xl p-4 shadow-soft">
      ...
      <?php if ($search !== ''): ?>
        <a href="<?= escape_html(site_url('admin/products.php')); ?>" class="text-sm font-semibold text-white/70 hover:text-white">Clear</a>
      <?php endif; ?>
      <button type="submit" class="inline-flex items-center gap-2 rounded-xl border border-white/20 px-4 py-2 text-sm font-semibold text-white hover:bg-white/10">
        Filter
      </button>
    </form>
      ...
      <div class="overflow-x-auto" data-admin-table-wrapper>
        <table class="admin-table min-w-full text-left text-sm" data-admin-table>
          ...
          <tbody class="divide-y divide-white/10 dark:divide-slate-800">
            <?php if (!$products): ?>
              <tr>
                <td colspan="7" class="px-4 py-6 text-center text-white/60">No products found.</td>
              </tr>
            <?php else: ?>
              <?php foreach ($products as $product): ?>
                <tr class="hover:bg-white/5">
                  <td class="px-4 py-4 font-semibold text-white">
                    ...
                    <p class="text-xs font-normal text-white/60">Slug: <?= escape_html($product['slug']); ?></p>
                  </td>
                  <td class="px-4 py-4 text-white/70"><?= escape_html($product['category'] ?: 'Uncategorized'); ?></td>
                  <td class="px-4 py-4 font-semibold text-white"><?= escape_html(format_currency($product['price_usd'], 'USD')); ?></td>
                  <td class="px-4 py-4 font-semibold text-white"><?= escape_html(format_currency($product['price_ngn'], 'NGN')); ?></td>
                  <td class="px-4 py-4">
                    <?php $statusLabel = $product['is_active'] ? 'Active' : 'Hidden'; $statusColor = $product['is_active'] ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/10 text-white/70'; ?>
                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold <?= $statusColor; ?>"><?= $statusLabel; ?></span>
                  </td>
                  <td class="px-4 py-4 text-white/60"><?= escape_html(date('M j, Y', strtotime($product['created_at']))); ?></td>
