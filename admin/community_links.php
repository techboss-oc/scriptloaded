...
<main class="flex-1 flex flex-col bg-white/40 dark:bg-slate-950/30">
  ...
  <section class="admin-content flex-1 space-y-8 overflow-y-auto px-4 py-6 sm:px-6 lg:px-10" data-admin-content>
    <div>
      <h1 class="text-3xl font-bold text-slate-900 dark:text-white">Community Links</h1>
      <p class="text-sm text-slate-500 dark:text-slate-400">Control the floating WhatsApp, Telegram, and YouTube shortcuts that appear across the site.</p>
    </div>
          ...
          <section class="admin-panel rounded-2xl p-6 shadow-soft dark:shadow-soft-dark">
            <div class="flex items-start justify-between gap-4">
              <div>
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400"><?= escape_html($meta['description']); ?></p>
              </div>
              <label class="inline-flex items-center gap-2 text-sm font-semibold text-slate-900 dark:text-white">
                <input type="checkbox" name="links[<?= escape_html($key); ?>][enabled]" value="1" class="h-5 w-5 rounded border-slate-400 text-primary focus:ring-primary/50" <?= $channel['enabled'] ? 'checked' : ''; ?> />
                Enable
              </label>
            </div>
